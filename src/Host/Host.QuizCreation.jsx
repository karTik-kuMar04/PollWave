    ...
    <div className="min-h-screen bg-[#121212] text-white p-6 md:p-10 overflow-hidden">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-3xl">
        ...
        {fields.map((field, index) => (
          <div key={field.id} className="border border-gray-700 p-4 rounded-md bg-[#1a1a1a] space-y-4 overflow-hidden">
            <div>
              ...
              {errors.questions?.[index]?.question && (
                <p className="text-red-500 text-sm mt-1">{errors.questions[index].question.message}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.questions?.[index]?.options && (
                <p className="text-red-500 text-sm mt-1">{errors.questions[index].options.message}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.questions?.[index]?.answer && (
                <p className="text-red-500 text-sm mt-1">{errors.questions[index].answer.message}</p>
              )}
            </div>
        ...
        <div className="flex gap-4">
          <button type="submit" className="bg-blue-600 hover:bg-blue-700 px-6 py-2 rounded font-semibold">
            Create Quiz
          </button>
          <button
            type="button"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          <div className="bg-[#1e1e1e] p-6 rounded-lg w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
            <div className="overflow-y-auto pr-2 flex-1">
              <h2 className="text-xl font-bold mb-4 sticky top-0 bg-[#1e1e1e] pb-2">Confirm Quiz Details</h2>
              ...
              <div className="mb-4">
                {formPreview.questions.map((q, i) => (
                  <div key={i} className="mb-3">
                    ...
                    <p className="text-green-400 text-sm mt-1">Correct Answer: Option {String.fromCharCode(65 + parseInt(q.answer))}</p>
                  </div>
                ))}
              </div>
            </div>
